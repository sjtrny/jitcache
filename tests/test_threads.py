...
def test_threads_store():
    ...
    assert store.get_value(json.dumps(kwarg_dict_1, sort_keys=True)) == slow_fn(**kwarg_dict_1)

    assert store.get_value(json.dumps(kwarg_dict_2, sort_keys=True)) == slow_fn(**kwarg_dict_2)